...
class ScanDossier
{
	public static function src($racine, $recurance = true)
	{
		...
		if(empty($arbo['fichiers']) && empty($arbo['dossiers']))
			return false;
		unset($arbo['pile_scan']);
		return $arbo;
	}
	...
	public static function sizeOfDir($src, $size = 0)
	{
		if(!file_exists($src))
			return false;
		$arbo = ScanDossier::src($src);
		if(empty($arbo))
			return false;
		foreach($arbo['fichiers'] AS $file)
			$size += (int) filesize($src.'/'.$file) / 1000;
		foreach($arbo['dossiers'] AS $dossier)
			$size += ScanDossier::sizeOfDir($src.$dossier.'/');
		return (int) $size;
	}
} ?>